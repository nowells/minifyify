...
*/
Minifier.prototype.registerMap = function (file, code, map, actualSrcPath) {
  this.registry[file] = {code:code, map:map, actualSrcPath:actualSrcPath};
};
...
Minifier.prototype.actualSrcPathForFile = function (file) {
    if(!this.fileExists(file)) {
        throw new Error('ENOFILE');
    }

    return this.registry[file].actualSrcPath;
};
...
Minifier.prototype.transformer = function (file) {
  ...
  write = function (data) {
    ...
    // Cannot find an embedded map. Try to find the actual source file.
    var dirPath = path.dirname(file);
    if (!existingMap) {
        try {
            existingMap = convertSM.fromMapFileSource(unminCode, dirPath);
        } catch(e) {
            // Something fucked up. Assume the source file's not there.
        }
    }
    existingMap = existingMap ? existingMap.toObject() : false;

    // This variable tell us where the source is actually from.
    // If it's embedded, it's the javascript name.
    // If it's from an external file, it's the external file path.
    // If it doesn't have any existing map, it's the javascript name.
    var actualSrcPath = file;
    ...
    } else if (existingMap && existingMap.sources && existingMap.sources.length) {
        // Source's not embedded. Read it from the source file.
        var sourcePath = path.normalize(dirPath + "/" + existingMap.sources[0]);
        try {
            originalCode = fs.readFileSync(sourcePath).toString();
            existingMap = JSON.stringify(existingMap);
            actualSrcPath = sourcePath;
        } catch(e) {
            // Can't find the file. Give up.
            existingMap = false;
        }
    } // Source's not embedded and external source isn't available. Assume it doesn't have an existing map.
    else {
      existingMap = false;
    }
    ...
    finish = function (tempExistingMapFile) {
      ...
      if(file.match(/\.json$/)) {
        ...
        if(self.opts.map) {
          self.registerMap(file, originalCode || unminCode, new SMConsumer(min.map), actualSrcPath);
        }
      }
...
Minifier.prototype.transformMap = function (bundleMap) {
  ...
  self.eachSource(function (file, code) {
    ...
    var offset = lineForSource(file) - 1
      ...
      , transformedFileName = self.opts.compressPath(self.actualSrcPathForFile(file));
